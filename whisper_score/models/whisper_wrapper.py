...
CACHE_DIR = os.path.expanduser("~/.cache/whisqa")
os.makedirs(CACHE_DIR, exist_ok=True)

MEL_URL = "https://github.com/pranoy-r/whisqa/releases/download/0.0.1/mel_filters.npz"
MEL_PATH = os.path.join(CACHE_DIR, "mel_filters.npz")
...
def mel_filters(device, n_mels: int = N_MELS) -> torch.Tensor:
    ...
    if not os.path.exists(MEL_PATH):
        print("Downloading mel_filters...")
        torch.hub.download_url_to_file(MEL_URL, MEL_PATH)

    with np.load(MEL_PATH, allow_pickle=True) as f:
        mel = torch.from_numpy(f[f"mel_{n_mels}"]).to(device)
        return mel
